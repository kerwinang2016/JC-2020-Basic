	...
	Views.Profile = Backbone.View.extend({
		...
		, submitProfile: function (e) {
			...
			if (measureTypeValue == 'Block') {
					...
					if(jQuery('[id*="body-fit"]').val() == 'Slim' && jQuery('[id*="body-block"]').val() != 'Select' && productTypeValue == 'Shirt'){
						if(parseFloat(jQuery('[id*="body-block"]').val()) < 35 || parseFloat(jQuery('[id*="body-block"]').val()) > 46){
							this.showError(_('You can only select slim profile for sizes 35 - 46').translate());
							return false;
						}
					}
			}
