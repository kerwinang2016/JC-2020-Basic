...
function service (request)
{
	...
	try
	{
		...
		{
			case 'GET':
				Application.sendContent(id ? ProductListItem.get(id,customerid) : ProductListItem.search(product_list_id, true, {
			...
			case 'POST':
				Application.sendContent(ProductListItem.create(data,customerid), {'status': 201});
			break;

			case 'PUT':
				ProductListItem.update(id, data, customerid);
				Application.sendContent(ProductListItem.get(id,customerid));
			break;

			case 'DELETE':

				ProductListItem.delete(id,customerid);
