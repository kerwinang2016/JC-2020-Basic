	...
	)
{
    ...
    ,	getFiles: function getFiles(url, files, optional_files, generate_file_paths)
		{
			var self = this
			,	result_promise;
			...
			try
			{
				...
				var promises = _.map(paged_files, function(files)
				{
					...
					if(get_files.length)
					{
						...
						//We use a POST to avoid the 502 http issue with Akamai
						var options = {
							url: url
						,	timeout: self.REQUEST_TIMEOUT
						,	method: 'POST'
						,	data: JSON.stringify({
							...
							,	files: get_files
							,	optional: optional
                            ,   generate_file_paths: generate_file_paths || false
							})
						};
    ...
    ,	normalizeFoldersPaths: function normalizeFoldersPaths(folders_paths, parent_name)
		{
			var self = this
			,	folders_map = {};

			folders_map[parent_name] = folders_paths.folder_id;

			_.each(folders_paths.sub_folders, function(folder, folder_name)
			{
				var folder_path = parent_name + '/' + folder_name;

				folders_map = _.extend(folders_map, self.normalizeFoldersPaths(folder, folder_path));
			});
