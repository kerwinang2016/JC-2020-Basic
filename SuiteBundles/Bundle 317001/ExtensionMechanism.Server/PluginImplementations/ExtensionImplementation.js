    ...
    return {
        ...
        validate : function(context)
        {
            ...
            try
            {
                ...
                var resourcesWithEntryPoints = ['javascript', 'sass', 'ssplibraries'];
                ...
                var resoucesWithApplication = ['javascript', 'templates'];
                _.each(resoucesWithApplication, function(resource)
                {
                    PluginImplementationHelper.validateApplication(sdf_object, resource);
                });

                var resources = _.union(resourcesWithEntryPoints, resoucesWithApplication, ['assets', 'configuration', 'suitescript2']);
        ...
        create : function(context)
        {
            ...
            try
            {
                ...
                if(context.input.deployment.mode === PluginImplementationHelper.DEVELOPMENT_MODE)
                {
                    ...
                    ExtensionHelper.createExtension(extension);
                }
        ...
        update : function(context)
        {
            ...
            try
            {
                ...
                if(context.input.deployment.mode === PluginImplementationHelper.DEVELOPMENT_MODE)
                {
                    ...
                    PluginImplementationHelper.updateExtensionRecord(extension);
                }
                else if(context.input.deployment.mode === PluginImplementationHelper.PRODUCTION_MODE)
                {
                    PluginImplementationHelper.validateVersion(sdf_object);
        ...
        serialize : function(context)
        {
            ...
            {
                var sdf_object = context.input.object
                ,   manifest_id = PluginImplementationHelper.getManifestIdByScriptId(sdf_object.scriptid)
                ,   manifest = File.load({id : manifest_id})
                ,   basepath = PluginImplementationHelper.getBasepath(manifest)
                ,   manifest_json = JSON.parse(manifest.getContents())
                ,   manifest_sdf_format = PluginImplementationHelper.convertManifestToSDFFormat(manifest_json, basepath);
