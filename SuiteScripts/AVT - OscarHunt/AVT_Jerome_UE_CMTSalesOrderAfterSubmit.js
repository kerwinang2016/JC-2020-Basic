...
function userEventAfterApproveSO(type) {
  if (type == 'approve' || type == 'edit') {
    if (type == 'edit') {
      ...
      if (oldRec.getFieldValue('orderstatus') == 'A' && (newRec.getFieldValue('orderstatus') == 'B')) {} else {
        return;
      }
    }
...
function pushliningarray(lining_arr, newSORecord, designoptionsname, liningname, name) {
  ...
  var iscmtlining = _.find(a, function(x) {
    return x.name == liningname
  });
  if (iscmtlining.value == 'CMT Lining') {
    var lc = _.find(a, function(x) {
      return x.name == "li-code"
    });
    var lining_code = lc ? lc.value : '';
    var lq = _.find(a, function(x) {
      return x.name == "li-qty"
    });
    var lining_quantity = lq ? lq.value : '';
    var lv = _.find(a, function(x) {
      return x.name == "li-vnd"
    });
...
function userEventAfterSubmitSO(type) {
  ...
  if ((type == 'create') && (executionContext == 'userinterface' || executionContext == 'webstore' || executionContext == 'restlet')) {
    try {
      ...
      nlapiLogExecution('debug','Producttypes',allproducttypes);
      ...
      if(allproducttypes.length>0){
        ...
        filter.push(new nlobjSearchFilter('custrecord_iscf_product_type', null, 'anyof', ptypeArr));
      }
      ...
      //    var innerfilter = [];
      //   for(var jj=0;jj<allproducttypes.length;jj++){
      //     // var f = new nlobjSearchFilter('custrecord_fis_producttype', null, 'is', allproducttypes[jj]);
      //     innerfilter.push(["custrecord_fis_producttype", "startswith", allproducttypes[jj]])
      //     if(jj != allproducttypes.length-1)
      //     innerfilter.push("OR");
      //     // if(jj == 0){
      //     //   f.leftparens = 1;
      //     // }
      ...
      nlapiLogExecution('debug','fabric_surcharges length', fabric_surcharges.length);
      filter = [], cols = [];
      filter.push(new nlobjSearchFilter('isinactive', null, 'is', 'F'));
      // if(allproducttypes.length>0){
      //   for(var jj=0;jj<allproducttypes.length;jj++){
      //     var f = new nlobjSearchFilter("custrecord_fis_producttype", "custrecord_fi_record", 'is', allproducttypes[jj]);
      //     f.isor = true;
      //     if(jj == 0){
      //       f.leftparens = 1;
      //     }
      ...
      for (var ii = 1; ii <= itemCount; ii++) {
        ...
        if ((type == 'create') && (executionContext == 'userinterface' || executionContext == 'webstore' || executionContext == 'restlet')) {
          ...
          if (newSORecord.getCurrentLineItemValue('item', 'povendor') == '78') {
              ...
              if (pickedVendor && pickedVendor != '') {
                switch (pickedVendor) {
                  ...
                  case '45': //Brisbane Moss
                    ...
                    break;
                  case '47': //Mileta
                    newSORecord.setCurrentLineItemValue('item', 'povendor', '1113');
                    break;
                  case '46': //Abraham Moons
                    newSORecord.setCurrentLineItemValue('item', 'povendor', '1109');
                    break;
                }
              }
            }
            ...
            switch (newSORecord.getCurrentLineItemValue('item', 'povendor')) {
              ...
              case '1085': //Brisbane Moss
                ...
                break;
              case '1109': //Brisbane Moss
                newSORecord.setCurrentLineItemValue('item', 'custcol_vendorpicked', '46');
                break;
              case '1113': //Brisbane Moss
                newSORecord.setCurrentLineItemValue('item', 'custcol_vendorpicked', '47');
                break;
            }
          }
          ...
          var arCustomColumnNames = ['custcol_ps_cart_item_id', 'custcol_custom_price', 'custcol_customer_amount', 'custcol_customer_rate', 'custcol_designoptions_jacket', 'custcol_designoption_message', 'custcol_designoptions_overcoat', 'custcol_designoptions_shirt', 'custcol_designoptions_trouser', 'custcol_designoptions_waistcoat', 'custcol_fabric_quantity', 'custcol_fitprofile_jacket', 'custcol_fitprofile_message', 'custcol_fitprofile_overcoat', 'custcol_fitprofile_shirt', 'custcol_fitprofile_trouser', 'custcol_fitprofile_waistcoat', 'custcol_fitprofile_summary', 'custcol_tailor_client_name', 'custcol_avt_date_needed', 'custcol_cmt_lining_text', 'custcol_producttype', 'custcol_vendorpicked', 'custcol_fitprofile_shirt_in', 'custcol_fitprofile_overcoat_in', 'custcol_fitprofile_waistcoat_in', 'custcol_fitprofile_trouser_in', 'custcol_fitprofile_jacket_in', 'custcol_fabric_extra', 'custcol_custom_fabric_details', 'custcol_designoptions_ladiesjacket', 'custcol_designoptions_ladiespants', 'custcol_designoptions_ladiesskirt', 'custcol_designoptions_ssshirt', 'custcol_designoptions_trenchcoat', 'custcol_fitprofile_ladiesjacket', 'custcol_fitprofile_ladiespants', 'custcol_fitprofile_ladiesskirt', 'custcol_fitprofile_ssshirt', 'custcol_fitprofile_trenchcoat', 'custcol_fitprofile_trenchcoat_in', 'custcol_fitprofile_ssshirt_in', 'custcol_fitprofile_ladiesskirt_in', 'custcol_fitprofile_ladiespants_in', 'custcol_fitprofile_ladiesjacket_in', 'custcol_designoptions_morning_coat', 'custcol_designoptions_shirt_jacket', 'custcol_designoptions_camp_shirt', 'custcol_designoptions_safari_jacket', 'custcol_designoptions_shorts', 'custcol_fitprofile_shorts_in', 'custcol_fitprofile_morning_coat_in', 'custcol_fitprofile_shirt_jacket_in', 'custcol_fitprofile_camp_shirt_in', 'custcol_fitprofile_safari_jacket_in', 'custcol_fitprofile_safari_jacket', 'custcol_fitprofile_camp_shirt', 'custcol_fitprofile_shirt_jacket', 'custcol_fitprofile_morning_coat', 'custcol_fitprofile_shorts'];
          if (isServiceItem != 'T') {
            ...
            if (ii > 1) {
              ...
              if (ptype) {
                clothingtype = clothingtypeObj[ptype];
                var custom = _.find(shippingcharges, function(x) {
                  return x.producttype == clothingtype && x.tailor == newSORecord.getFieldValue('entity')
                });
                if (custom) {
                  ...
                  custom = _.find(shippingcharges, function(x) {
                    return x.producttype == clothingtype && x.tailor == ''
                  });
                  if (custom) {
                    shippingamount = parseFloat(shippingamount) + parseFloat(custom.rate);
                    newSORecord.setCurrentLineItemValue('item', 'custcol_shippingprice', parseFloat(custom.rate));
