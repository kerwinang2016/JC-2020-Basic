 ...
 */
 ...
 define(['N/record', 'N/search', 'N/log', 'N/format', 'ustyylit/integration'], 
	function (record, search, log, format, ustyylit){
		...
		function createOrder(order){
			...
			var shopname = rec.getText('entity').replace("'","").replace("&","");
			...
			var receiveOrderData = {
				"Action": "CreateOrder",
				"orders": {"Order":[orderObj]}
			}
			console.log(receiveOrderData);
			//log.debug('cs createOrder receiveOrderData',JSON.stringify(receiveOrderData));
			var responseData = ustyylit.receiveOrder(receiveOrderData);
			log.debug('cs createOrder responseData',JSON.stringify(responseData));
			//return orderObj;
		}
